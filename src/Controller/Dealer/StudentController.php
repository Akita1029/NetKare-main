...
class StudentController extends AbstractController
{
    ...
    #[Route('/', name: 'dealer_student_index', methods: ['GET'])]
    public function index(Request $request, StudentRepository $studentRepository, Search $search, Pagination $pagination, SchoolRepository $schoolRepository): Response
    {
        $school = $search->getSchool();

        if (!$school) {
            return $this->render('dealer/student/index.html.twig');
        }
        ...
        if ($filter->isSubmitted() && $filter->isValid()) {
            $students = $studentRepository->getStudentPaginator($offset, $this->getUser(), $school, $filter->get('classroom')->getData());
        } else {
            $students = $studentRepository->getStudentPaginator($offset, $this->getUser(), $school);
        }
    ...
    #[Route('/new', name: 'dealer_student_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager, Search $search): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('dealer_student_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->renderForm('dealer/student/new.html.twig', [
            'student' => $student,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'dealer_student_show', methods: ['GET'])]
    public function show(Student $student): Response
    {
        $this->checkStudentOwner($student);

        return $this->render('dealer/student/show.html.twig', [
            'student' => $student,
        ]);
    }

    #[Route('/{id}/edit', name: 'dealer_student_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, Student $student, AlbumPhotoRepository $albumPhotoRepository, ImageUploader $imageUploader, EntityManagerInterface $entityManager): Response
    {
        ...
        $formBuilder = $this->createFormBuilder([
            ...
            ->add('student', StudentType::class, [
                'has_school' => false,
                'school' => $student->getClassroom()->getSchool()
            ])
        ...
        return $this->renderForm('dealer/student/edit.html.twig', [
            'student' => $student,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'dealer_student_delete', methods: ['POST'])]
