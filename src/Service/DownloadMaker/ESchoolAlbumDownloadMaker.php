...
class ESchoolAlbumDownloadMaker extends ZipMaker implements DownloadMakerInterface
{
    const FORMAT = 'jpeg';
    ...
    public function make(School $school): string
    {
        foreach ($school->getClassrooms() as $classroom) {
            ...
            foreach ($albumPhotos as $albumPhoto) {
                ...
                $name = $school->getName() . '/' . $classroom->getName() . '/' . $student->getSchoolNumber() . '.' . self::FORMAT;
