...
class Biometric4DownloadMaker extends ZipMaker implements DownloadMakerInterface
{
    const FORMAT = 'jpeg';
    ...
    public function make(School $school): string
    {
        foreach ($school->getClassrooms() as $classroom) {
            ...
            foreach ($albumPhotos as $albumPhoto) {
                ...
                $name = $student->getClassroom()->getName() . '/' . $student->getSchoolNumber() . ' ' . $student->getName() . ' ' . $student->getSurname() . '.' . self::FORMAT;

                $content = $this->biometric4->create([$albumPhoto]);
