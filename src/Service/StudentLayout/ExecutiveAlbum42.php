...
class ExecutiveAlbum42 extends StudentLayout implements StudentLayoutInterface
{
    const GRID_ROW = 6;
    const GRID_COL = 7;
    ...
    const COLLAGE_PADDING_TOP = 167;
    const COLLAGE_PADDING_LEFT = 104;

    const IMAGE_W = 310;
    const IMAGE_H = 413;

    const IMAGE_MARGIN_TOP = 100;
    const IMAGE_MARGIN_LEFT = 17;
    ...
    public function create(Classroom $classroom, array $students): string
    {
        ...
        if (count($students) > self::GRID_COL * self::GRID_ROW) {
            throw new Exception();
        }
